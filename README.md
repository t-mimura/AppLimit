...
ブラウザ等から直接インストールした場合、Android 14 以降の **制限付き設定**（Restricted Settings）機能により、`使用状況へのアクセス`・`他のアプリの上に表示` の権限を直接許可できないことがある。許可しに行こうとするとグレーアウトされて操作不能になる症状が起きる。
...
AppLimit 起動時の権限設定画面でも、Android 14 以降のサイドロード版を検出した場合に同様のガイドと「アプリ情報を開く」ボタンを表示する。
